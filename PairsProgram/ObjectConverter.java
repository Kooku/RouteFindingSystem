...
public class ObjectConverter {
	 ...
	 * fuck the comments
	 */
	public static baseMapFile readFile() {
		try {
			FileInputStream fin = new FileInputStream("rfs.dat");
			ObjectInputStream ois = new ObjectInputStream(fin);
			baseMapFile bs = (baseMapFile) ois.readObject();
			return bs;
		} catch (Exception e) {
	...
	public static void saveFile(baseMapFile bs) {
		try {
			FileOutputStream fout = new FileOutputStream("rfs.dat");
	...
	public static SystemManager getSystem() throws Exception {
		File f = new File("rfs.dat");
		if(!(f.exists())) {
			f.createNewFile();
			ObjectConverter.saveFile(new baseMapFile());
		}
		
		baseMapFile bs = ObjectConverter.readFile();
		
		SystemManager sm = bs.addData();
		return sm;
	}
